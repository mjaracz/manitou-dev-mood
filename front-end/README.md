For users how use Linux system or other architectur based for UNIX kernel.
Before you allow to use script form package.json. Must to do is install react-script globaly for commond

>sudo npm install react-script -g

(optional step)
Next, in my opinion very inportant thing is added environment variable by linking whole PATH where our's npm was installed.
For example you may use this command:

>echo PATH
>>\>exports /our/path/were/npm/is/installed 
...
[`src/index.ts`](src/index.js) first by modifying the service worker import:
```javascript
